...
(
   ...
   sale_type            smallint comment 
			   			'类别取值含义：
						1：销售单
						-1：退款单',
   sale_status          smallint comment 
			   			'销售单状态如下：
						0：待付款
						1：已付款',
   primary key (sale_ID)
...
(
   ...
   studio_sale_count    int,
   studio_position      varchar(50),
   studio_introduction  varchar(2000),
   studio_flag          smallint comment    
   						'取值含义：
...
alter table studio AUTO_INCREMENT=20170500;
*/
...
alter table sale add constraint FK_user_sale foreign key (user_id)
...
insert into user(user_name) values ("临时用户");
